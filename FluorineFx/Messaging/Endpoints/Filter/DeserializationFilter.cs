...
namespace FluorineFx.Messaging.Endpoints.Filter
{
    ...
    private static readonly ILog log = LogManager.GetLogger(typeof(DeserializationFilter));
    /// <summary>
      ...
      int read;
      while ((read = context.InputStream.Read(buffer, 0, buffer.Length)) > 0)
      {
      ...
      //AMFDeserializer deserializer = new AMFDeserializer(context.InputStream);
		...
		#endregion
    private void logBytes(byte[] codes)
    {
