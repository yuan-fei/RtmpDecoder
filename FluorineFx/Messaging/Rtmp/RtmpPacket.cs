...
namespace FluorineFx.Messaging.Rtmp
{
    ...
    public sealed class RtmpPacket
	{
	  ...
	  internal RtmpPacket(RtmpHeader header)
		{
			_header = header;
			MessageData = ByteBuffer.Allocate(header.Size + (int)header.RtmpHeaderDescription.BytesRange.RangeByteCount);
      ChuncksData = ByteBuffer.Allocate(1024);
      ChuncksData.AutoExpand = true;
		}
	  ...
	  /// </summary>
	  public ByteBuffer MessageData { get; set; }

	  public ByteBuffer ChuncksData {get; set; }
        /// <summary>
        ...
        private string ToStringFields(int indentLevel)
        {
            ...
            value += sep + "data = " + (MessageData != null ? "buffer(" + MessageData.Length + ")" : "(null)");
            return value;
        }
