    ...
    class RtmpClientConnection : RtmpConnection
    {
        ...
        public override void Write(RtmpPacket packet)
        {
            if (!IsClosed)
            {
                ...
                //byte[] bytes = GetMockBytes((RtmpPacket) packet);
                //if(bytes!=null)
                //{
                //  Write(bytes);
                //  RtmpProtocolEncoder.LogEncodedBytes(bytes);
                //}
                //else
                //{
                  Write(outputStream);
        ...
        private bool mockStarted = false;
        private int counter = -1;
        private byte[] GetMockBytes(RtmpPacket packet)
        {
          log.Debug("GetMockBytes");
          var header = packet.Header;
          IRtmpEvent message = packet.Message;
          if (!mockStarted)
          {
            if (header.DataType == Constants.TypeSharedObject)
            {
              foreach (var Event in ((ISharedObjectMessage) message).Events)
              {
                if (Event.Type == SharedObjectEventType.SERVER_SET_ATTRIBUTE)
                {
                  mockStarted = true;
                  //header.Timer = 0xfffffe;
                  //header.Timer = 0xffffff + new Random().Next(0xffff);
                  break;
                }
              }
            }
            if (!mockStarted) return null;
          }
          counter++;
          return MockData.MockbyteArrays[counter];
        }
